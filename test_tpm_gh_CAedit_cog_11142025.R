...
cogtable_gen_tpm <- read_tsv("/Users/colleenahern/Documents/Magda_BPs_experiment/metatranscriptomics/MT_grouped_reads/COGtable_tpm_11102025.tsv")
cogtable_gen_tpm <- cogtable_gen_tpm %>%
...
# 11/19/2025 COG groups test
result <- aov(value ~ condition_cba, data = subset(ff, cat == "N")) # PHA + G1 + AS t1 vs. PHA + AS t1 padj = 0.0464746
summary(result)                                                     # PHA + G1 + AS t2 vs. PHA + AS t2 padj = 0.0231928
TukeyHSD(result)

result <- aov(value ~ condition_cba, data = subset(ff, cat == "C")) # PHA + G1 + AS t1 vs. PHA + AS t1 padj = 0.0002871
