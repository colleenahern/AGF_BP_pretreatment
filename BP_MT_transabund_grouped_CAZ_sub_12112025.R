...
# This time use ANCOMBC2 instead of ANCOMBC because it is more conservative and considered more accurate
...
dbcantable_gen_rawrt_filt <- dbcantable_gen_rawrt_filt[,-19]
# ── Run two contrasts ─────────────────────────────────────────────────────────
res_S3 <- run_ancombc(dbcantable_gen_rawrt_filt, bpcoldata3,
                      cond1_labels = "CA_S3_AS",
                      cond2_labels = "CA_AS")

res_G1 <- run_ancombc(dbcantable_gen_rawrt_filt, bpcoldata3,
...
volc_mat <- cbind(
  CAS3AS_lfc = res_S3$lfc_contrasttreatment,
  CAS3AS_padj = res_S3$q_contrasttreatment,
  PHAG1AS_lfc = res_G1$lfc_contrasttreatment,
  PHAG1AS_padj = res_G1$q_contrasttreatment
)
rownames(volc_mat) <- res_S3$taxon
volc_mat <- as.data.frame(volc_mat)
volc_mat$CAZid <- rownames(volc_mat)
write_tsv(volc_mat, "/Users/colleenahern/Documents/Magda_BPs_experiment/KEGG_mods/CAZ_subs/df_bothcomps_CAZymelfcpadj_ancombc2_04292026.tsv")
volc_matr <- read_tsv("/Users/colleenahern/Documents/Magda_BPs_experiment/KEGG_mods/CAZ_subs/df_bothcomps_CAZymelfcpadj_ancombc2_04292026.tsv")
...
volc_matr$diffexpressed_cas3as <- "NO"

# if log2Foldchange > 0.6 and pvalue < 0.05, set as "UP"
volc_matr$diffexpressed_cas3as[volc_matr$CAS3AS_lfc > 1 & volc_matr$CAS3AS_padj < 0.05] <- "UP"

# if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
volc_matr$diffexpressed_cas3as[volc_matr$CAS3AS_lfc < -1 & volc_matr$CAS3AS_padj < 0.05] <- "DOWN"
...
dlup <- volc_matr[volc_matr$diffexpressed_cas3as == "UP",]
volc_matr$delabel <- ifelse(volc_matr$CAZyme %in% as.matrix(head(dlup[order(dlup$CAS3AS_padj), "CAZyme"], 10)), volc_matr$CAZyme, NA)

dldown <- volc_matr[volc_matr$diffexpressed_cas3as == "DOWN",]
volc_matr$delabel <- ifelse(volc_matr$CAZyme %in% as.matrix(head(dldown[order(dldown$CAS3AS_padj), "CAZyme"], 10)), volc_matr$CAZyme, volc_matr$delabel)

np1b <- ggplot(data = volc_matr, aes(x = CAS3AS_padj_lfc, y = -log10(CAS3AS_padj), col = diffexpressed, label = delabel)) +
