# 03/19/2025
# Colleen Ahern
# ANCOM-BC on my MT data for differential abundance testing of the active bacteria
...
library(microbiome)
################################################################################################################
### In the processed tables folder that Malte gave to me, there are two taxa files (for metaassembly, and two for clustering, I'll use the
### metaassebmly now): gtdb and NR. I'm not sure what the difference is? Look in Malte's pipeflow or email him

### Let's start with the gtdb file

bpcts <- readr::read_tsv("/Users/colleenahern/Documents/Magda_BPs_experiment/metatranscriptomics/MT_analysis/MT_analysis/processed_tables/read_counts_gene_metassembly.tsv")
taxgtdb <- readr::read_tsv("/Users/colleenahern/Documents/Magda_BPs_experiment/metatranscriptomics/MT_analysis/MT_analysis/processed_tables/tax_gtdb_metassembly.tsv")

bpcts2 <- bpcts[,-c(1,3:6)]

bpcts_joined <- aggregate(. ~ contig, data = bpcts2, sum)

rownames(bpcts_joined) <- bpcts_joined$contig
otumat <- bpcts_joined[, -1]
taxmat <- as.data.frame(taxgtdb)
rownames(taxmat) <- taxmat$contig
taxmat <- taxmat[,-1]
...
bpcoldata3 <- bpcoldata2[!(rownames(bpcoldata2) %in% drops),]
...
sig_taxa = tab_diff %>%
  dplyr::filter(`groupCA + S3 + AS` == TRUE) %>%
  .$taxon
...
df_heat_pos <- df_heat[df_heat$value>3,]
p_heat = df_heat_pos %>%
